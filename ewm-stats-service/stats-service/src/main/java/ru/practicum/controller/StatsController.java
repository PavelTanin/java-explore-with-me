...
public class StatsController {
    ...
    @PostMapping("/hit")
    public ResponseEntity addHit(@RequestBody EndpointHitDto endpointHitDto) {
        log.info("Получен POST-запрос от прирожения {}", endpointHitDto.getApp());
        return new ResponseEntity(new String[]{statsService.addHit(endpointHitDto)}, HttpStatus.CREATED);
    }

    @GetMapping("/stats")
    @ResponseStatus(HttpStatus.OK)
    public ResponseEntity getStats(@RequestParam(name = "start") String start,
